...
import org.example.mock.bean.{CityInfo, ProductInfo, UserInfo, UserVisitAction}
import org.example.mock.util.config.PropertiesUtil
import org.example.mock.util.{RanOpt, RandomDate, RandomNum, RandomOptions}

import scala.collection.mutable.ListBuffer

/**
 * Created by atyongsi@163.com on 2020/8/19
 * Description:模拟数据后插入到 hive里
 */
object MockerOffline {
  ...
  val searchKeywordsOptions = RandomOptions(RanOpt("手机", 30), RanOpt("笔记本", 70), RanOpt("内存", 70), RanOpt("i7", 70), RanOpt("苹果", 70), RanOpt("吃鸡", 100))
    ...
    for (i <- 1 to sessionNum) {
      ...
      while (!isQuit) {
        ...
        if (action == "quit") {
          ...
          val actionDateTime = randomDate.getRandomDate() //随机产生时间戳
